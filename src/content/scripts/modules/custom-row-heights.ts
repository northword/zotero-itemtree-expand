 ...
 * 计算单个条目应该有的行高
 */
function calculateRowHeight(row: Element, win: Window): number {
 ...
 */
export function generateCustomRowHeights(win: Window, forceRecalculate = false): number[][] {
  try {
    ...
    ztoolkit.log(`[Custom Row Heights] Generated ${customHeights.length} heights (${newCalculations} new, ${cachedUsed} cached, total cache: ${heightCache.size})`);
 ...
 */
export function applyCustomRowHeights(win: Window, forceRecalculate = false): void {
  try {
      ...
      if (itemTree) {
        ...
        if (lastContainerWidth !== currentWidth) {
          if (isRecentResize && widthDiff < 20) {
            // 刚完成resize，忽略小幅变化
            ztoolkit.log(`[Custom Row Heights] Ignoring small width change ${lastContainerWidth} → ${currentWidth} (Δ${widthDiff}px) after recent resize`);
            lastContainerWidth = currentWidth;
            return; // 直接返回，不强制重算
          } else {
            ztoolkit.log(`[Custom Row Heights] Width changed: ${lastContainerWidth} → ${currentWidth}, forcing recalculation`);
            lastContainerWidth = currentWidth;
    ...
    if (itemsView.tree && typeof itemsView.tree.updateCustomRowHeights === 'function') {
      itemsView.tree.updateCustomRowHeights(customHeights);
      ztoolkit.log(`[Custom Row Heights] Applied ${customHeights.length} custom row heights`);
  ...
  try {
    ...
    const resizeObserver = new win.ResizeObserver((entries: any) => {
      for (const entry of entries) {
        ...
        // 避免拖动窗口时每1px变化都触发
        if (widthChange >= 10) {
          ztoolkit.log(`[Custom Row Heights] Significant width change: ${lastContainerWidth} → ${newWidth} (Δ${widthChange}px)`);
          ...
          (win as any).__resizeTimeout = win.setTimeout(() => {
            ztoolkit.log(`[Custom Row Heights] Width stabilized at ${newWidth}, resetting to initial state`);
            
            // 标记正在resize，防止applyCustomRowHeights中的宽度检测干扰
            isResizing = true;
            
            const ZoteroPane = (win as any).ZoteroPane;
            if (!ZoteroPane || !ZoteroPane.itemsView || !ZoteroPane.itemsView.tree) {
              isResizing = false;
              return;
            }
            ...
            if ((win as any).__customHeightsMutationObserver) {
              ...
              ztoolkit.log(`[Custom Row Heights] MutationObserver paused for resize`);
            }
            ...
            if (typeof tree.updateCustomRowHeights === 'function') {
              ...
              ztoolkit.log(`[Custom Row Heights] Cleared all custom row heights`);
            }
            ...
            win.setTimeout(() => {
              ...
              win.setTimeout(() => {
                ztoolkit.log(`[Custom Row Heights] Starting fresh calculation from top`);
                ...
                win.setTimeout(() => {
                  ...
                  // 恢复MutationObserver
                  const tableBody = itemTree?.querySelector('.virtualized-table-body');
                  if (tableBody && (win as any).__customHeightsMutationObserver) {
                    (win as any).__customHeightsMutationObserver.observe(tableBody, {
                      childList: true,
                      subtree: true,
                      attributes: false
                    });
                    ztoolkit.log(`[Custom Row Heights] MutationObserver resumed`);
                  }
                  
                  ztoolkit.log(`[Custom Row Heights] Resize complete, cache has ${heightCache.size} items`);
                }, 100);
              }, 50);
            }, 100);
          }, 500);
    ...
    const mutationObserver = new win.MutationObserver((mutations: any) => {
      ...
      for (const mutation of mutations) {
        if (mutation.type === 'childList') {
          const added = Array.from(mutation.addedNodes).filter((node: any) => 
            node.classList && node.classList.contains('row')
          );
          const removed = Array.from(mutation.removedNodes).filter((node: any) => 
            node.classList && node.classList.contains('row')
          );
    ...
    ztoolkit.log("[Custom Row Heights] Observers set up successfully (ResizeObserver + smart MutationObserver)");
